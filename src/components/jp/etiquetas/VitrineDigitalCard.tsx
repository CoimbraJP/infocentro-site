...
import type { VitrineDigitalPayload } from '@/lib/jp/vitrine-digital';

const LOGO_SRC = '/jp/info-centro-mark.png';

// Tamanho fixo (não relativo à tela) — é o que garante que a imagem baixada
// (PNG, via html-to-image) e a exibição ao vivo no tablet (que só amplia
// isso pra caber na tela, ver VitrineDigitalDisplay) sejam sempre
// pixel-a-pixel a mesma coisa, gerada uma única vez aqui.
export const VITRINE_DIGITAL_CARD_WIDTH = 480;
export const VITRINE_DIGITAL_CARD_HEIGHT = 960;

interface VitrineDigitalCardProps {
  payload: VitrineDigitalPayload;
}

/** Visual único da Vitrine Digital — usado tanto pra exportar como imagem quanto pra exibir ao vivo num tablet. */
export default function VitrineDigitalCard({ payload }: VitrineDigitalCardProps) {
  ...
  return (
    <div
      style={{ width: VITRINE_DIGITAL_CARD_WIDTH, height: VITRINE_DIGITAL_CARD_HEIGHT }}
      className="flex flex-col bg-[#0a0a0d] text-white"
    >
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-4">
        {payload.foto ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={payload.foto} alt={payload.marcaModelo} className="max-h-[380px] w-auto object-contain" />
        ) : (
